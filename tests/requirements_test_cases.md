## FR-HADJ-01~05 - 헤더 수동 보정/반복 사용
| HADJ01-TC1 | 1차 추정 UI 표시 | 파일 추가 시 헤더 가인식 후 표 UI 표시 | 사용자에게 헤더 영역이 강조된 표가 보임 |
| HADJ01-TC2 | 안내 문구 확인 | 보정 UI 진입 시 | “헤더로 쓸 행을 드래그하거나 시작/끝 행을 조정하세요” 문구 노출 |
| HADJ02-TC1 | 드래그로 헤더 위치 변경 | 표에서 헤더 영역을 드래그/선택 | 시작/끝 행이 변경되어 미리보기 갱신 |
| HADJ02-TC2 | 정확한 행 수 반영 | 2행을 드래그로 선택 | 확정 시 정확히 2행만 헤더로 저장됨(과다 선택 없음) |
| HADJ02-TC3 | 스핀버튼 조정 | ↑/↓로 시작/끝 행 변경 | 각 클릭마다 정확히 1행씩만 변경되어 선택 반영 |
| HADJ02-TC4 | 시작/끝 스핀 독립성 | 시작↕, 끝↕ 연속 조작 | 서로 값이 묶이지 않고 범위만 맞춰 반영(시작<끝 유지) |
| HADJ02-TC5 | 시작행을 끝행 아래로 내릴 때 | 시작행↓가 끝행보다 커지면 | 끝행이 따라 내려가 범위 유지 |
| HADJ02-TC6 | 끝행을 시작행 위로 올릴 때 | 끝행↑가 시작행보다 작아지면 | 시작행이 따라 올라가 범위 유지 |
| HADJ02-TC7 | 스핀/드래그 시 하이라이트 | 시작/끝 스핀(↑/↓) 또는 드래그로 구간 변경 | 선택된 행 하이라이트 색이 즉시 새로운 범위에 적용됨 |
| HADJ02-TC8 | 끝행 스핀 조정 시 전체 하이라이트 | 헤더 끝행 스핀(↓)으로 범위 확대 | 시작~끝 모든 행이 하이라이트되고 끝행만 단독으로 선택되지 않는다 |
| HADJ02-TC9 | 시작행 스핀 조정 시 하이라이트 갱신 | 헤더 시작행 스핀(↑/↓) 또는 드래그로 변경 | 변경된 시작행부터 끝행까지 하이라이트가 즉시 새 범위로 반영된다 |
| HADJ03-TC1 | 헤더 확정 저장 | 헤더 확정 후 다른 파일 추가 | 저장된 헤더 위치/데이터가 내부에 기록 |
| HADJ03-TC2 | 헤더 재선택 가능 | “헤더 다시 선택” 실행 | 이미 확정된 헤더라도 UI를 통해 재조정 가능 |
| HADJ04-TC1 | 동일 헤더 자동 적용 | 동일 서식 파일 재추가 | 팝업 없이 자동 적용 |
| HADJ05-TC1 | 다른 헤더 재추정 | 다른 서식 파일 추가 | 추정 결과를 표 UI로 띄우고 사용자가 수정 가능 |
| HADJ06-TC1 | 전체 초기화 후 재입력 | 파일 모두 제거 → 새 파일 추가 | 다시 시트 선택/헤더 선택 과정이 강제된다 |
| HADJ07-TC1 | 첫 파일 추가 | 비어 있는 상태에서 첫 파일 드롭 | 헤더 선택창 1회 표시 후 시그니처 저장 |
| HADJ07-TC2 | 저장된 헤더와 동일 | 이후 파일이 저장된 헤더 데이터와 동일 | 헤더 선택창 없이 자동 적용 |
| HADJ07-TC3 | 저장된 헤더와 다름 | 이후 파일이 저장된 헤더와 다른 행 데이터 | 자동 추정값으로 보정 UI 표시, 사용자가 수정 가능 |
| HADJ07-TC4 | 동일 헤더 재파싱/파일 제거 | 기존 헤더 저장 후 동일 헤더 파일만 남긴 상태에서 재로딩 | 헤더 선택창 없이 자동 적용(팝업 없음) |
| HADJ09-TC1 | 시트별 헤더 확인 버튼 | 파일 목록의 시트 행에서 “헤더 확인/조정” 클릭 | 해당 시트만 보정 UI가 열리고 결과가 시트별로 저장 |
| HADJ09-TC2 | 시트별 저장 재사용 | 동일 시트에서 재로딩 | 시트별로 저장된 헤더 시그니처/밴드가 우선 적용, 팝업 없음 |
| HADJ09-TC3 | 다른 시트는 별도 보정 | 같은 파일 내 다른 시트 클릭 | 다른 시트는 별도 보정 UI가 열려 독립적으로 저장 |


## FR-HEADER# 요구사항별 테스트 케이스 매트릭스
...
| HADJ02-TC9 | ??? ?? ?? ? ????? ?? | ?? ??? ??(?/?) ?? ???? ?? | ??? ????? ???? ?????? ?? ? ??? ???? |
 ??/??? ? ????? | ??/? ??(?/?) ?? ???? ?? ?? | ??? ? ????? ?? ?? ??? ??? ??? |
 끝행을 시작행 위로 올릴 때 | 끝행↑가 시작행보다 작아지면 | 시작행이 따라 올라가 범위 유지 |
| HADJ03-TC2 | 헤더 재선택 가능 | “헤더 다시 선택” 실행 | 이미 확정된 헤더라도 UI를 통해 재조정 가능 |
| HADJ03-TC1 | 헤더 확정 저장 | 헤더 확정 후 다른 파일 추가 | 저장된 헤더 위치/데이터가 내부에 기록 |
| HADJ04-TC1 | 동일 헤더 자동 적용 | 동일 서식 파일 재추가 | 팝업 없이 자동 적용 |
| HADJ05-TC1 | 다른 헤더 재추정 | 다른 서식 파일 추가 | 추정 결과를 표 UI로 띄우고 사용자가 수정 가능 |
...
| HADJ07-TC3 |
| HADJ07-TC4 | ?? ?? ???/?? ?? | ?? ?? ?? ? ?? ?? ??? ?? ???? ??? | ?? ??? ?? ?? ??(?? ??) |
 저장된 헤더와 다름 | 이후 파일이 저장된 헤더와 다른 행 데이터 | 자동 추정값으로 보정 UI 표시, 사용자가 수정 가능 |
